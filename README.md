...
- 🧠 **本地大模型**：基于 Ollama + DeepSeek 8B，消费级GPU可流畅运行
...
|重排序 |CrossEncoder (bge-reranker-v2-m3)| 精排候选文档，提升相关性|
...
原本只有向量检索，当用户问“退款流程”这种关键词明确的问题时，向量检索可能抓不准。加了 BM25 关键词检索后，两路结果合并去重，解决了纯向量检索对精确关键词不敏感的问题。

### Rerank 精排

多路召回后候选文档较多，用 Rerank 模型对候选文档重新打分排序，把最相关的文档排在前面，再送入大模型生成回答，提升回答质量。
...
注：由于本地网络环境限制，Docker 构建时拉取基础镜像超时。Dockerfile 已配置使用国内镜像源加速，在稳定网络环境下可正常运行。
