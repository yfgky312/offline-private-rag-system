...
## 💬 面试常见问题**Q：为什么用 Chroma 而不是 Milvus？**Chroma 轻量级，适合快速验证和中小规模知识库。如果数据量超过百万级，我会迁移到 Milvus，它支持分布式索引和更复杂的检索策略。**Q：混合检索是怎么实现的？**向量检索负责语义匹配，BM25 负责关键词匹配。两者各自返回 Top 3 结果，合并去重后送入大模型，兼顾语义理解和精确匹配。**Q：如何降低大模型幻觉？**- 检索阶段：通过混合检索提高召回质量- 生成阶段：强制要求模型基于提供的文档内容回答，禁止编造- 溯源阶段：返回引用原文，用户可以自行核对**Q：Docker 部署有什么好处？**将环境和代码打包成镜像，任何机器上只需执行 `docker run` 即可运行，无需手动安装 Python、依赖和模型，适合企业私有化交付。
